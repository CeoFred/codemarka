...
const updateCurrentMessageThread = (state, action) => {
    ...
    if (
        state.messageThread.showingThread &&
        state.messageThread.messageId === incomingThreadMessageID
    ) {
        return helper.updateObject(state, {
            messageThread: {
                ...state.messageThread,
...
export default (state = INITIAL_STATE, action) => {
    switch (action.type) {
        case actionTypes.CLASSROOM_CREATE_INIT:
            return classroomCreationInit(state, action)
        case actionTypes.CLASS_CREATION_SUCCESS:
            return classroomCreatedSuccess(state, action)
        case actionTypes.CLASS_CREATION_FAILED:
            return classroomCreationFailed(state, action)
        case actionTypes.CLASSROOM_JOINED:
            return classroomJoined(state, action)
        case actionTypes.CLASSROOM_LEFT:
            return classroomLeft(state, action)
        case actionTypes.CLASSROOM_VERIFICATION_INIT:
            return classroomVerifyStart(state, action)
        case actionTypes.CLASSROOM_VERIFICATION_SUCCESS:
            return classroomVerified(state, action)
        case actionTypes.CLASSROOM_VERIFICATION_FAILED:
            return classroomVerificationFailed(state, action)
        case actionTypes.CLASSROOM_RESET:
            return resetClassRoomData(state, action)
        case actionTypes.SET_DEFAULT_INPUT_OUTPUT_DEVICES:
            return setDefaultInputOutputDevices(state, action)
        case actionTypes.SET_INPUT_OUTPUT_DEVICES:
            return setInputOutputDevices(state, action)
        case actionTypes.MESSAGE_THREAD_DATA_SET_DONE:
            return setThreadData(state, action)
        case actionTypes.MESSAGE_THREAD_FETCH_DONE:
            return fetchedMessageThreadSuccessfull(state, action)
        case actionTypes.MESSAGE_THREAD_UPDATED:
            return updateCurrentMessageThread(state, action)
        case actionTypes.SET_DISPLAYING_MESSAGE_REACTION_PICKER:
            return showMessageReactionPicker(state, action)
        case actionTypes.SET_CLASSROOM_SOCKET_CONNECTION:
            return setSocketConnection(state, action)
        case actionTypes.CLOSE_MESSAGE_REACTION_EMOJI_PICKER:
            return closeMessageReactionPicker(state, action)
        case actionTypes.SET_EDIT_OR_DELETE_MESSAGE_DATA:
            return setEditOrDeleteMessageData(state, action)
        case actionTypes.UNSET_EDIT_OR_DELETE_MESSAGE_DATA:
            return unsetMessageEditOrDeleteData(state, action)
        default:
            return state
    }
}
